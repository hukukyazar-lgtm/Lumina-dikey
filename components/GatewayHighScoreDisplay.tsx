...
interface GatewayHighScoreDisplayProps {
  gateway: number;
}

const GatewayHighScoreDisplay: React.FC<GatewayHighScoreDisplayProps> = ({ gateway }) => {
  ...
  return (
    <div className={`h-10 flex items-center justify-center ${panelClasses} backdrop-blur-sm border px-4 sm:px-6 shadow-bevel-inner rounded-lg animate-appear`} style={{animationDelay: '0.5s'}}>
      <span className={`text-lg sm:text-xl font-bold text-brand-accent-secondary`}>
        🕳️{gateway}
      </span>
    </div>
  );
...
export default React.memo(GatewayHighScoreDisplay);